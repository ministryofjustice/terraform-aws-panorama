...
locals {
  ...
  logger_panoramas = { for panorama in var.panoramas : panorama.name => panorama if contains(keys(panorama), "ebs") && panorama.ebs != null }
...
resource "aws_instance" "this" {
  ...
  lifecycle {
    ignore_changes = [
      ami, ebs_block_device, key_name, aws_eip
    ]
  }
}
